...
utils.unique = (...args) => [...new Set(utils.flatten(...args))];

utils.union = (init, ...rest) => {
  let arr = utils.flatten(rest);
  for (let ele of arr) {
    if (ele != null && !init.includes(ele)) {
      init.push(ele);
    }
  }
  return init;
...
utils.modules = (str, options) => {
  return utils.requires(str, options).concat(utils.imports(str, options));
};

utils.omit = (obj = {}, keys = []) => {
  return utils.filterObj(obj, key => !keys.includes(key));
};
