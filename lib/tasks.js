...
module.exports = argv => {
  ...
  return app => {
    ...
    app.task('lint', function() {
      ...
      // console.log(app.report.dependencies.files[0].modules);
      // process.exit()
    ...
    app.task('add', async () => {
      const type = /^(D|devDependencies)$/.test(argv.type)
        ? 'devDependencies'
        : 'dependencies';
    ...
    app.task('prompt-unused', async () => {
      ...
      for (const file of app.report.dependencies.files) {
        // console.log(file.path, file.modules)
      }
