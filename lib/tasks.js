...
const bold = (msg, ...rest) => console.log(log.bold(msg), ...rest);
const error = console.log.bind(console, log.error);
const info = console.log.bind(console, log.info);
    ...
    app.task('why', function() {
      ...
      if (report.count === 0) {
        if (report.types.length) {
          error(`${log.bold(name)} is specified in "${report.types.join(', ')}" but is not used anywhere`);
          console.log(`  in your project (run ${log.dim('$ deps clean')} to remove unused deps)`);
        } else {
          error(`${log.bold(name)} is not used anywhere in ${pkgName}, and is not defined in package.json.`);
        }
      } else {
        const types = Object.keys(report.files);
        let files = [];
        types.forEach(function(type) {
          files = files.concat(report.files[type]);
        });

        const list = rpt.format(files, { color: 'yellow', type: 'inline' });
        const singular = utils.singularize(report.types[0]);
        info(`${name} is a ${log.cyan(singular)}, used in ${list}`);
      }
      return Promise.resolve(null);
    });
...
function getTasks(app, argv) {
  const names = pick(argv, Object.keys(app.tasks));
  const arr = Object.keys(names).filter(name => !!argv[name]);
